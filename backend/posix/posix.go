...
//
// Every bucket and object is addressed by a path built from the absolute
// root directory (see BucketPath and ObjectPath). The process working
// directory is read once, to resolve a relative rootdir, and never changed,
// so the backend can be embedded in a process whose working directory
...
func New(rootdir string, ms meta.MetadataStorer, opts PosixOpts) (*Posix, error) {
	...
	// bucket names the same way this backend does: under the root directory.
...
// BucketPath returns the filesystem path of the bucket directory. Bucket
// names are resolved under the root directory, never the process working
// directory.
//
...
func (p *Posix) BucketPath(bucket string) string {
	...
	return filepath.Join(p.rootdir, bucket)
}
...
func (p *Posix) ListBuckets(ctx context.Context, input s3response.ListBucketsInput) (s3response.ListAllMyBucketsResult, error) {
	...
	fis, err := listBucketFileInfos(p.rootdir, p.bucketlinks)
	if err != nil {
